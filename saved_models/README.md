...
|----------------|-----------------------------------------------------------------------------|
| `WRF`          | Trained with WRF (Weather Research and Forecasting) atmospheric forcings.   |
| `MODIS`        | Trained with MODIS (Moderate Resolution Imaging Spectroradiometer) data, specifically snow cover and Leaf Area Index (LAI). |
| `STATIC`       | Trained with static variables, including elevation and temporal encoding.   |
| `vX`           | A version number indicating different training runs (e.g., `v1`, `v2`).    |
